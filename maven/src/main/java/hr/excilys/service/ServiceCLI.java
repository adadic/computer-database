...
public class ServiceCLI {
	...
	public void computerGetDetail() throws SQLException {
		...
		if (choice > 0) {
			...
			if (comp.isPresent()) {
				...
				logger.warn("Computer does not exist\n");
		}
	}
	...
	private int insertChoice(String name, long introducedTime, long discontinuedTime, int company_id)
			throws SQLException {

		if (introducedTime != 0) {
			if (discontinuedTime != 0) {
				return daoComputer.insertComputer(new Computer.ComputerBuilder(name)
						.introduced(new Timestamp(introducedTime)).discontinued(new Timestamp(discontinuedTime))
						.company(new Company.CompanyBuilder(company_id, "").build()).build());
			} else {

				return daoComputer
						.insertComputer(new Computer.ComputerBuilder(name).introduced(new Timestamp(introducedTime))
								.company(new Company.CompanyBuilder(company_id, "").build()).build());
			}
		} else {
		...
		if (computer.isPresent()) {
			...
			if (introducedTime >= 0) {
				...
				if (discontinuedTime > introducedTime) {
					...
					return daoComputer.updateComputer(new Computer.ComputerBuilder(name).id(id).introduced(introduced)
							.discontinued(new Timestamp(discontinuedTime))
							.company(new Company.CompanyBuilder(company_id, "").build()).build());
				} else if (discontinuedTime == 0) {

					return daoComputer.updateComputer(
							new Computer.ComputerBuilder(name).id(id).introduced(introduced).discontinued(discontinued)
									.company(new Company.CompanyBuilder(company_id, "").build()).build());
				} else {
	...
	public void computerUpdate() throws SQLException {
		...
		if (result == 1) {
			...
			logger.error("Computer does not exist\n");
	}
